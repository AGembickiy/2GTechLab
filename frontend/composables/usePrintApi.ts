...
export function usePrintApi() {
  ...
  const base = computed(() => {
    // Используем прокси-путь для разработки
    return '/api';
  });
  ...
  async function startSlice(
    ...
    body?: {
      ...
      dimensions?: { width_mm: number; height_mm: number; thickness_mm: number } | null;
    },
  ): Promise<{ status: string; job_id: number }> {
    return await $fetch(`${base.value}/print-jobs/${id}/slice/`, {
      method: 'POST',
      body: body ?? {},
    });
  }
  ...
  async function uploadFile(file: File): Promise<UploadResponseDto> {
    ...
    return await $fetch<UploadResponseDto>(`${base.value}/upload/`, {
      method: 'POST',
      body: form,
    });
  }
  ...
  async function getPrintJobResult(id: number): Promise<PrintJobResultDto> {
    return await $fetch<PrintJobResultDto>(`${base.value}/print-jobs/${id}/result/`);
  }

  async function createOrder(file: File, params: { infill: number; layer_height: number; material: string }): Promise<any> {
    const body = new FormData();
    body.append('original_file', file, file.name);
    ...
    // Добавляем заголовки для корректной обработки вложенных данных, если используется multipart
    return await $fetch(`${base.value}/orders/`, {
      method: 'POST',
      body,
    });
  }
