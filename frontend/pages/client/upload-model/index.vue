  ...
  <div class="space-y-6">
    <div class="rounded-2xl border border-slate-800/60 bg-slate-900/40 p-6">
      <div class="mb-4 flex items-center justify-between">
        ...
        <AppButton color="primary" @click="handleFileSelect">
          Выбрать файл
        </AppButton>
      </div>
      ...
      >
        <Icon name="i-heroicons-cloud-arrow-up" class="mx-auto h-12 w-12 text-slate-500" />
      ...
      <div v-else class="mt-4 rounded-lg border border-slate-800 bg-slate-950 p-4">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            <Icon name="i-heroicons-document" class="h-6 w-6 text-cyan-400" />
            <div>
              <p class="font-semibold text-slate-200">{{ file.name }}</p>
              <p class="text-sm text-slate-400">{{ formatSize(file.size) }}</p>
            </div>
          </div>
            <AppButton color="red" variant="soft" @click="file = null">
              Удалить
            </AppButton>
        </div>
      </div>
      ...
      <div v-if="file" class="mt-6 space-y-4">
        ...
        <div v-if="uploadKind === 'sketch'" class="grid gap-4 sm:grid-cols-3">
          <div>
            <AppInput label="Ширина (мм)" v-model="sketchWidth" type="number" step="0.1" />
          </div>
          <div>
            <AppInput label="Высота (мм)" v-model="sketchHeight" type="number" step="0.1" />
          </div>
          <div>
            <AppInput label="Толщина (мм)" v-model="sketchThickness" type="number" step="0.1" />
          </div>
        </div>
